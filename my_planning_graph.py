...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            for s_node in a_node.effnodes:
                p = set()
                if s_node in connections:
                    p = connections[s_node]
                p.add(a_node)
                connections[s_node] = p
                a_node.children.add(s_node)

        for s_node, a_nodes in connections.items():
            s_node.parents = a_nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        def one_add_other_remove(one, other):
            for eff_pos_1 in one.effect_add:
                if eff_pos_1 in other.effect_rem:
                    return True
            return False
        # check for effect that was added in a1 and removed in a2
        if one_add_other_remove(node_a1.action, node_a2.action):
            return True
        # check for effect that was added in a2 and removed in a1
        if one_add_other_remove(node_a2.action, node_a1.action):
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        def one_effect_negate_other(one, other):
            for precond_pos in one.precond_pos:
                if precond_pos in other.effect_rem:
                    return True
            for precond_neg in one.precond_neg:
                if precond_neg in other.effect_add:
                    return True
            return False

        if one_effect_negate_other(node_a1.action, node_a2.action):
            return True
        if one_effect_negate_other(node_a2.action, node_a1.action):
            return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for p_action1 in node_s1.parents:
            for p_action2 in node_s2.parents:
                if not p_action1.is_mutex(p_action2):
                    return False
